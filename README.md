<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=250&color=gradient&customColorList=12,20,24,30&text=Simge%20Özlem%20Sarp&fontSize=50&fontColor=fff&animation=twinkling&fontAlignY=38&desc=Backend%20Developer%20•%20Computer%20Engineer&descAlignY=60"/>

<h1 align="center">Hi 👋 I'm Simge</h1>

<p align="center">
<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=1000&color=A020F0&center=true&vCenter=true&width=700&lines=Backend+Software+Engineer;Java+%7C+Spring+Boot+%7C+Microservices;Apache+Kafka+%7C+Docker+%7C+AWS;Graduated+First+in+Class+%F0%9F%8F%86;Always+Learning+New+Things"/>
</p>

<p align="center">
<img src="https://komarev.com/ghpvc/?username=simgeozlemsarp&label=Profile%20Views&color=blueviolet&style=flat"/>
</p>

---

## 🚀 About Me

- 🎓 Computer Engineering graduate — **Fenerbahçe University** (GPA: 3.44 / 4.00, **First in Class** 🏆)
- 💻 Backend Software Engineer — experienced in **Java, Spring Boot, Microservices**
- ⚡ Worked on a multi-tenant sustainability platform using **Apache Kafka & Docker & AWS**
- 🔧 Interned at **Qoex Technology**, **Pointek Elektrik-Elektronik**, **Logiverse Technology**
- 🤖 Interested in Embedded Systems & Hardware (CAN Bus, ESP32, Verilog)
- 🌱 Strong believer in clean architecture, SOLID principles & quality code

---

## 💻 Tech Stack

**Backend**
<p align="center">
<img src="https://skillicons.dev/icons?i=java,spring,dotnet,python,php&theme=dark"/>
</p>

**Database & DevOps**
<p align="center">
<img src="https://skillicons.dev/icons?i=postgresql,mysql,docker,kafka&theme=dark"/>
</p>

**Tools & Other**
<p align="center">
<img src="https://skillicons.dev/icons?i=git,github,vscode,postman,figma&theme=dark"/>
</p>

---

## 💼 Experience

| Period | Role | Company |
|--------|------|---------|
| Dec 2025 – Apr 2026 | Software Development Engineer | Qoex Technology |
| Jun 2024 – Aug 2024 | Hardware Development Intern | Pointek Elektrik-Elektronik |
| Oct 2023 – Feb 2025 | Part-Time Student Assistant | Fenerbahçe University UZEM |
| Jun 2023 – Aug 2023 | Software Development Intern | Logiverse Technology |

---

## 🏗️ Projects

| Project | Description | Stack |
|---------|-------------|-------|
| 🩺 **E-Diabetic** | Web-based diabetes patient monitoring system with doctor/patient dashboards | PHP, JS, MySQL |
| 🌡️ **Temp & Humidity Monitor** | IoT system with ESP32 + DHT22 sensor over WiFi | C++, ESP32 |
| 💰 **Bank ATM Simulation** | ATM app simulating banking operations | Python |
| 🔢 **ALU Design** | Arithmetic Logic Unit design & simulation | Verilog |

---

## 🎓 Education & Achievements

- 🏛️ **Fenerbahçe University** — Computer Engineering, B.Sc. *(Sep 2020 – Jul 2025)*
- 🏆 Graduated **First in Class** — Faculty of Engineering and Architecture (GPA: 3.44 / 4.00)
- 🎖️ Fenerbahçe University **Honor Certificate**
- 📜 **TÜBİTAK BİLGEM YTE Bootcamp 2023** — Java, Spring Boot, Microservices, UX, Figma
- 📜 TS EN ISO 14064-1:2019 Greenhouse Gas Emissions Calculation and Reporting Certificate

---

## 🌐 Languages

| Language | Level |
|----------|-------|
| 🇹🇷 Turkish | Native |
| 🇬🇧 English | B2 Upper-Intermediate |

---

## 🌐 Connect With Me

<p align="center">
<a href="https://linkedin.com/in/simgeozlemsarp">
<img src="https://img.shields.io/badge/LinkedIn-6a11cb?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>
<a href="mailto:simgeozlemsarp@gmail.com">
<img src="https://img.shields.io/badge/Gmail-6a11cb?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>
</p>

---

## 🐍 Contribution Snake

<p align="center">
<img src="https://raw.githubusercontent.com/simgeozlemsarp/simgeozlemsarp/output/github-contribution-grid-snake-dark.svg"/>
</p>
